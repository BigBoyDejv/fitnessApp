...
export default function ClassesTab({ setActiveTab }) {
  ...
  const [forceWaitingIds] = useState(() => {
        ...
        {loading ? (
          ...
          <div className="timeline-feed">
            {classes.map((item, idx) => {
              const hasPos = item.waitlistPosition && item.waitlistPosition > 0;
              const isWaiting = !item.isReserved && (item.isWaiting || forceWaitingIds.has(String(item.id))) && hasPos;
              ...
              return (
                <React.Fragment key={c.id}>
                  ...
                  <div className={`timeline-card ${isPast ? 'past' : 'active'} ${c.isWaiting ? 'waiting' : ''}`}>
                    ...
                    <div className="t-content-card">
                      <div className="t-header">
                        ...
                        <div className="t-status">
                          {c.isWaiting ? (
                            <span style={{ color: 'var(--orange)' }}>ČAKÁ SA ({c.waitlistPosition}. v poradí)</span>
                          ) : isPast ? (
                            'UKONČENÁ'
                          ) : (
                            <span style={{ color: 'var(--acid)' }}>AKTÍVNA</span>
                          )}
                        </div>
                      </div>
